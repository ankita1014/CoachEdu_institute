...
const GoogleSignInButton = ({ mode = 'signin', isAdmin = false, onError }) => {
  ...
  const handleGoogleAuth = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      try {
        ...
        if (data.success) {
          ...
          if (onError)
            onError('Login failed: ' + (data.message || 'Unknown error'));
        }
      } catch (error) {
    ...
    onError: (error) => {
      if (onError)
        onError('Failed to login with Google. Check console for details.');
    },
  });

  return (
    <button onClick={handleGoogleAuth} className="google-btn" type="button">
      <svg
        width="18"
        height="18"
        viewBox="0 0 18 18"
